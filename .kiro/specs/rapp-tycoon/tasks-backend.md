...
- [~] 6.1 Implement `RappCatalogueService.getCatalogue()` — returns all rApp templates
- [~] 6.2 Implement `RappFactory.createDeployment(templateId, basestationId, playerId)` — Factory pattern: creates RappDeployment from template with DEPLOYING status
- [~] 6.3 Implement `RappBehaviour` interface with method `calculateImpact(configuration): MetricImpact`
- [~] 6.4 Implement Strategy pattern classes for each rApp type:
...
- [~] 6.5 Implement aggressiveness multiplier in each behaviour (LOW=0.5×, MODERATE=1.0×, HIGH=1.5×)
- [~] 6.6 Implement `CatalogueController` with endpoint: `GET /api/rapps/catalogue`
- [~] 6.7 Implement DTOs: `RappTemplateResponse`, `ImpactDto`
